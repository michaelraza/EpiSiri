...
from routers.router_auth import get_current_Customer
router = APIRouter(
    prefix='/products',
    tags=['Products']
)
def is_admin(Customer: models.Customers = Depends(get_current_Customer)):
    # Check if the Customer is an admin
    if not Customer.is_admin:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='Insufficient permissions')

def has_permission(Customer: models.Customers = Depends(get_current_Customer)):
    # Check if the Customer has permission
    if not Customer.has_permission:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='Insufficient permissions')
...
async def delete_product(product_id: int, cursor: Session = Depends(get_cursor), Customer: models.Customers = Depends(is_admin)):
...
async def update_product(product_id: int, payload: schemas.Products_PATCH_Body, cursor: Session = Depends(get_cursor), Customer: models.Customers = Depends(is_admin)):
